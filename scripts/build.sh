...
CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=${project_version}" -o "${output_dir}/local-parental-control-indicator" "${project_dir}/cmd/local-parental-control-indicator"
chmod 0755 "${output_dir}/local-parental-control" "${output_dir}/lpctl" "${output_dir}/local-parental-control-indicator"
